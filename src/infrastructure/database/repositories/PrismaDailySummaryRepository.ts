...
export class PrismaDailySummaryRepository implements IDailySummaryRepository {
    ...
    try {
      ...
      await this.prisma.dailySummary.upsert({
        ...
        create: {
          ...
          userId: summary.userId,
          date: normalizedDate,
          totalEarnings: summary.earnings.value,
          totalExpenses: summary.expenses.value,
          profit: summary.profit.value,
          km: summary.km.value,
          fuelExpenses: 0, // TODO: calcular do expenses
          otherExpenses: summary.expenses.value,
          costPerKm: summary.costPerKm?.value ?? null,
          tripsCount: 0, // TODO: contar trips
          weeklyGoal: null,
  ...
  private toDomain(row: any): DailySummary {
    return DailySummary.restore({
      ...
      userId: row.userId,
      date: row.date,
      earnings: Money.create(Number(row.totalEarnings)),
      expenses: Money.create(Number(row.totalExpenses)),
      profit: Money.create(Number(row.profit)),
      km: Distance.create(row.km),
      costPerKm: row.costPerKm ? Money.create(Number(row.costPerKm)) : null,
